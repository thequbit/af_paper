...
function calculate(name,noise)
    ...
    sums1d32 = zeros(32,1);
    sums1d64 = zeros(32,1);
    sums1d128 = zeros(32,1);

    sums2d32 = zeros(32,1);
    sums2d64 = zeros(32,1);
    sums2d128 = zeros(32,1);

    sums1dfft32 = zeros(32,1);
    sums1dfft64 = zeros(32,1);
    sums1dfft128 = zeros(32,1);
    
    sums2dfft32 = zeros(32,1);
    sums2dfft64 = zeros(32,1);
    sums2dfft128 = zeros(32,1);

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	printf("Calculating focus values using 1D Difference method for a 32x32 box ...\n");
	sums1d32 = getsums1d(name,32,noise);

	printf("Calculating focus values using 1D Difference method for a 64x64 box ...\n");
    sums1d64 = getsums1d(name,64,noise);

	printf("Calculating focus values using 1D Difference method for a 128x128 box ...\n");
	sums1d128 = getsums1d(name,128,noise);


	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	printf("Calculating focus values using 2D Difference method for a 32x32 box ...\n");
	sums2d32 = getsums2d(name,32,noise);

	printf("Calculating focus values using 2D Difference method for a 64x64 box ...\n");
	sums2d64 = getsums2d(name,64,noise);

	printf("Calculating focus values using 2D Difference method for a 128x128 box ...\n");
	sums2d128 = getsums2d(name,128,noise);


	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	printf("Calculating focus values using 1D FFT method for a 32x32 box ...\n");
	sums1dfft32 = getsums1dfft(name,32,noise);

	printf("Calculating focus values using 1D FFT method for a 64x64 box ...\n");
	sums1dfft64 = getsums1dfft(name,64,noise);

	printf("Calculating focus values using 1D FFT method for a 128x128 box ...\n");
	sums1dfft128 = getsums1dfft(name,128,noise);


	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	printf("Calculating focus values using 2D FFT method for a 32x32 box ...\n");
	sums2dfft32 = getsums2dfft(name,32,noise);

	printf("Calculating focus values using 2D FFT method for a 64x64 box ...\n");
	sums2dfft64 = getsums2dfft(name,64,noise);

	printf("Calculating focus values using 2D FFT method for a 128x128 box ...\n");
	sums2dfft128 = getsums2dfft(name,128,noise);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	printf(strcat("Writing out calculated values to '",name,".csv' ...\n\n"));
	...
	fputs(fid,"1D Diff 32x32, 2D Diff 32x32, 1D FFT 32x32, 2D FFT 32x32, 1D Diff 64x64, 2D Diff 64x64, 1D FFT 64x64, 2D FFT 64x64, 1D Diff 128x128, 2D Diff 128x128, 1D FFT 128x128, 2D FFT 128x128,\n");
	for i = 1:32
		
		line = "";
		line = strcat(line, sprintf("%.0f",sums1d32(i)),",",   sprintf("%.0f",sums2d32(i)),",",  sprintf("%.0f",sums1dfft32(i)),",",  sprintf("%.0f",sums2dfft32(i)),",");
		line = strcat(line, sprintf("%.0f",sums1d64(i)),",",   sprintf("%.0f",sums2d64(i)),",",  sprintf("%.0f",sums1dfft64(i)),",",  sprintf("%.0f",sums2dfft64(i)),",");
		line = strcat(line, sprintf("%.0f",sums1d128(i)),",",  sprintf("%.0f",sums2d128(i)),",", sprintf("%.0f",sums1dfft128(i)),",", sprintf("%.0f",sums2dfft128(i)),",\n");
...
calculate("playing_card",0);
calculate("playing_card",1);

calculate("business_card",0);
calculate("business_card",1);
